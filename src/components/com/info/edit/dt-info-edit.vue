<template>
  <dt-info-edit-text
    v-if="option.text"
    :value="val === null || val === undefined || val === '' ? option.placeholder : val"
    :option="option.text" />
  ...
  <div v-else-if="option.date">
    <el-date-picker
      ...
      :teleported="option.date.teleported != null ? option.date.teleported : false" />
    <div class="tips" v-html="option.tips"></div>
  </div>
  <div v-else-if="option.TimePicker">
    <el-time-picker
      ...
      @change="onChange"
      :editable="option.editable || false"
      :teleported="option.TimePicker.teleported != null ? option.TimePicker.teleported : false"
      append-to="body"
      split-panels />
    <div class="tips" v-html="option.tips"></div>
  </div>
  ...
  <div v-else-if="option.select" style="width: 100%">
    <el-select
      ref="select"
      :teleported="option.select.teleported != null ? option.select.teleported : false"
      append-to="body"
  ...
  <div v-else-if="option.treeSelect">
    <el-tree-select
      ...
      :placeholder="option.placeholder"
      @change="onChange"
      :data="option.treeSelect.data"></el-tree-select>
    <div class="tips" v-html="option.tips"></div>
  </div>
          ...
          <div class="demo_upload_list">
            ...
            <div
              ...
              <el-icon
                ...
                @click.native="preViewImgRemove(info[option.key], index)">
                <Delete />
              </el-icon>
            </div>
          </div>
  ...
  <div v-else-if="option.upload">
    ...
    <div v-if="option.upload.showDel" style="max-height: 200px; overflow-y: auto">
      <div
        ...
        <el-button type="primary" link style="color: #2d8cf0" @click="removeFileFun(info[option.key], index)">
          删除
        </el-button>
      </div>
    </div>
  ...
  <div v-else-if="option.file">
    ...
    <div v-for="(item, index) in val" :key="index" style="display: flex">
      <p>{{ item.name }}</p>
      <p v-if="item.tip" style="margin-left: 20px">{{ item.tip }}</p>
      <el-button type="primary" link style="color: #2d8cf0" @click="() => option.file.click(item)">
        下载
      </el-button>
    </div>
  </div>
    ...
    methods: {
      ...
      onUploadBefore(file) {
        ...
        if (
          ...
          this.option.uploadImg.files.length >= this.option.upload.maxlength
        ) {
