<template>
    <div class="select" v-if="choices.length > 0">
        ...
        <el-button type="primary" class="reset" @click="onReset"
            >重置</el-button
        >
    </div>
    <div v-else></div>
...
const [current, currentInfo, choices, infos] = [
    ref(),
    ref({}),
    ref([]),
    ref({}),
...
function onOK() {
    if (
        ...
        !current.value.filter.check ||
        current.value.filter.check(filter.value.info(current.value.key))
    ) {
...
function update() {
    ...
    console.log(choices.value, "9999");
