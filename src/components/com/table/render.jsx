...
function hGroup(title, width, cells) {
	return (
		<div style={{ width: `${width}px`, textAlign: "center" }}>
			{title && (
				<div class={"dt-table-header-title"} style={{ lineHeight: "48px" }}>
					{title}
				</div>
			)}
			<div style={{ display: "flex", height: `${title ? 48 : 96}px` }}>{cells}</div>
		</div>
	);
}
...
function img(opt, numLength) {
	return [
		h("img", {
			style: {
				display: opt.data?.length > 0 || opt.src ? "inline-block" : "none",
				// width: '50px',
				height: "50px",
				textAlgn: "center",
				cursor: "pointer"
...
function option(opt, field, row) {
	if (opt.text) {
		let value = opt.value ? opt.value(row) : field && row ? dt.tool.getVal(row, field) : opt.placeholder;
		return text({
			...opt.text,
			value
		});
