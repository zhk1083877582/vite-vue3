    ...
    columns.forEach((column, idx) => {
        if (column._header_group_title) {
            ...
            if (
                ...
                columns[idx + 1]._header_group_title != title
            ) {
                groupCells.push(render.header.group(title, width, tmpCells));
            }
        } else {
            groupCells.push(
                render.header.group("", column.width, [cells[idx]])
            );
        }
    });
    return groupCells;
...
const checkbox = reactive({
    ...
    click: (row) => {
        if (row) {
            ...
            props.checkbox?.onOne &&
                props.checkbox.onOne(row, row[checkboxKey]);
        } else {
        ...
        nextTick(
            () =>
                props.checkbox?.onChange &&
                props.checkbox.onChange(checkboxChoices, checkboxUnChoices)
        );
    },
    disabled: (row) => {
        if (row) {
            row._checkbox_disabled = props.checkbox?.isDisabled
                ? props.checkbox.isDisabled(row)
                : false;
            return row._checkbox_disabled;
        } else {
            return props.checkbox?.radio
                ? true
                : props.checkbox?.isDisabled
                ? props.checkbox.isDisabled()
                : false;
        }
    },
...
function handleWidth(column) {
    if (column.type == "checkbox" || column.type == "index")
...
function handleTxt(column) {
    if (column.dataKey.indexOf("_txt_") < 0)
        column.dataKey = "_txt_" + column.dataKey;
    return column;
}
...
function onSort(res) {
    ...
    Object.keys(sortState.value).forEach(
        (key) => (sortState.value[key] = key == res.key ? res.order : "asc")
    );
    let info = {};
    info[res.column.sort.key ?? res.key] =
        res.column.sort[res.order] ?? res.order;
    emits("sort", info);
}
...
function handleColumn(column) {
    ...
    if (!column.hidden) {
        ...
        if (column.cell) {
            if (
                column.cell.text &&
                !textMgr.hasStyle(column.cell.text) &&
                column.field
            ) {
...
function fixWidth() {
    ...
    if (rest - widthInfo.min > 0) {
        ...
        avg = avg + rest / minL;
        widthInfo.minItems.forEach(
            (item) => (item.width = Math.max(Number(item.minWidth), avg))
        );
    }
}
...
function handleTxtData(item) {
    ...
    textItems.forEach((opt) => {
        let val = opt.cell?.value
            ? opt.cell.value(item)
            : dt.tool.getVal(item, opt.field);
...
function choiceCheckbox(item, val) {
    if (checkboxKey) {
        if (val != null) {
            ...
            item[checkboxKey] = choice
                ? choice.all
                    ? true
                    : choice.values?.indexOf(item[choice.key]) > -1
                : false;
            item._checkbox_disabled = props.checkbox?.isDisabled
                ? props.checkbox.isDisabled(item)
                : false;
        }
    }
}
