<template>
    <div
        ...
        :style="{
            justifyContent:
                filterItems.length > 0 ? 'space-between' : 'flex-end',
        }"
    >
        <dt-filter
    ...
    <div
        ref="contentRef"
        style="position: relative; overflow: hidden; box-sizing: content-box"
        :key="keyIdx"
    >
        <dt-table
            ref="tableRef"
...
function loadOpt(Opt) {
    ...
    opts.forEach((item) => {
        ...
        if (item.sort?.value && item.visible != false) {
            sortInfo.value[item.sort.key || item.field] =
                item.sort[item.sort.value] || item.sort.value;
        }
    });
